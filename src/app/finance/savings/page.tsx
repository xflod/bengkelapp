...
import { PlusCircle, Edit3, Trash2, Eye, DollarSign, CalendarIcon as CalendarDateIcon, Hourglass, BookOpen, Info } from "lucide-react"; // Changed PiggyBank to BookOpen
...
export default function SavingsBookPage() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data: goalsData, error: goalsError } = await supabase.from('savings_goals').select('*').order('created_at', { ascending: false }); 
      ...
      setSavingsTransactions(trxData.map(t => ({...t, id: String(t.id), goal_id: String(t.goal_id)})) as SavingsTransaction[]);
    } catch (error: any) {
  ...
  const handleOpenGoalForm = useCallback((goal?: SavingsGoal) => { if (goal) { setEditingGoal(goal); setGoalName(goal.name); setGoalTargetAmount(goal.target_amount); setGoalStartDate(goal.start_date && isValid(parseISO(goal.start_date)) ? parseISO(goal.start_date) : startOfDay(new Date())); setGoalTargetDate(goal.target_date && isValid(parseISO(goal.target_date)) ? parseISO(goal.target_date) : undefined); setGoalNotes(goal.notes || ''); } else { resetGoalForm(); } setIsGoalFormOpen(true); }, [resetGoalForm]);
  ...
  const handleSaveGoal = async () => {
    ...
    if (editingGoal && editingGoal.id) {
      const currentAmount = editingGoal.current_amount; 
  ...
  const handleSaveTransaction = async () => {
    ...
    if (transactionType === 'Penarikan' && parsedAmount > selectedGoalForTransaction.current_amount) { toast({ variant: "destructive", title: "Penarikan Melebihi Saldo" }); return; }
    ...
    const newCurrentAmount = selectedGoalForTransaction.current_amount + amountToChange;
    const newStatus = newCurrentAmount >= selectedGoalForTransaction.target_amount ? 'Tercapai' : 'Aktif';
  ...
  const calculateAverageDailySavings = useCallback((goal: SavingsGoal) => { const goalTransactions = savingsTransactions.filter(t => t.goal_id === goal.id && t.type === 'Setoran'); if (goalTransactions.length === 0 || !goal.start_date || !isValid(parseISO(goal.start_date))) return 0; const totalSavedForGoal = goalTransactions.reduce((sum, t) => sum + t.amount, 0); const daysSinceStart = differenceInDays(new Date(), parseISO(goal.start_date)) + 1; return daysSinceStart > 0 ? totalSavedForGoal / daysSinceStart : 0; }, [savingsTransactions]);
  const calculateEstimatedDaysToGoal = useCallback((goal: SavingsGoal) => { if (goal.status === 'Tercapai' || goal.current_amount >= goal.target_amount) return "Tercapai!"; const averageDailySaving = calculateAverageDailySavings(goal); if (averageDailySaving <= 0) { const goalTransactions = savingsTransactions.filter(t => t.goal_id === goal.id && t.type === 'Setoran'); return goalTransactions.length > 0 ? "N/A (progres tdk positif)" : "Belum ada setoran."; } const remainingAmount = goal.target_amount - goal.current_amount; if (remainingAmount <= 0) return "Tercapai!"; const estimatedDays = Math.ceil(remainingAmount / averageDailySaving); if (!isFinite(estimatedDays) || estimatedDays <= 0) return "Data tdk cukup."; const futureDate = addDays(new Date(), estimatedDays); return `~${estimatedDays} hari (Estimasi: ${format(futureDate, "dd MMM yyyy", { locale: localeID })})`; }, [calculateAverageDailySavings, savingsTransactions]);
  ...
  const sortedGoals = useMemo(() => [...savingsGoals].sort((a, b) => { if (a.status === 'Aktif' && b.status !== 'Aktif') return -1; if (a.status !== 'Aktif' && b.status === 'Aktif') return 1; return new Date(b.created_at).getTime() - new Date(a.created_at).getTime(); }), [savingsGoals]);
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && savingsGoals.length === 0 ? (<Card className="w-full max-w-md mx-auto text-center shadow-lg border-dashed py-10"><CardHeader className="items-center"><BookOpen className="w-16 h-16 text-muted-foreground mb-4" /><CardTitle className="text-xl text-foreground">Belum Ada Target</CardTitle></CardHeader><CardContent><p className="text-muted-foreground">Klik "Tambah Target Baru".</p></CardContent></Card>) : (
        <div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {sortedGoals.map(goal => (<Card key={goal.id} className="shadow-lg hover:shadow-xl flex flex-col"><CardHeader><div className="flex justify-between items-start"><CardTitle className="text-xl text-primary">{goal.name}</CardTitle>{getGoalStatusBadge(goal.status)}</div><CardDescription>Target: Rp {goal.target_amount.toLocaleString()}</CardDescription></CardHeader><CardContent className="flex-grow space-y-3"><div><div className="flex justify-between items-center mb-1"><span className="text-sm text-muted-foreground">Terkumpul:</span><span className="text-sm font-semibold text-foreground">Rp {goal.current_amount.toLocaleString()}</span></div><Progress value={(goal.current_amount / goal.target_amount) * 100} className="h-3" /><p className="text-xs text-muted-foreground mt-1 text-right">{(((goal.current_amount / goal.target_amount) * 100).toFixed(1))}%</p></div><div className="p-2 bg-muted/50 rounded-md"><p className="text-xs text-muted-foreground">Estimasi Capai:</p><p className="text-sm font-medium text-accent-foreground">{goal.status === 'Tercapai' ? 'Tercapai!' : calculateEstimatedDaysToGoal(goal)}</p></div><p className="text-xs text-muted-foreground">Mulai: {format(parseISO(goal.start_date), "dd MMM yyyy", { locale: localeID })}</p>{goal.target_date && <p className="text-xs text-muted-foreground">Target Selesai: {format(parseISO(goal.target_date), "dd MMM yyyy", { locale: localeID })}</p>}</CardContent><CardFooter className="grid grid-cols-2 gap-2 border-t pt-4"><Button variant="outline" onClick={() => handleOpenTransactionForm(goal)} disabled={goal.status === 'Tercapai' || goal.status === 'Dibatalkan'} className="col-span-2"><DollarSign className="mr-2 h-4 w-4" /> Transaksi</Button><Button variant="ghost" size="sm" onClick={() => handleOpenHistoryDialog(goal)}><Eye className="mr-2 h-4 w-4" /> Riwayat</Button><Button variant="ghost" size="sm" onClick={() => handleOpenGoalForm(goal)}><Edit3 className="mr-2 h-4 w-4" /> Edit</Button><Button variant="destructive" size="sm" onClick={() => handleDeleteGoal(goal.id)} className="col-span-2 mt-1"><Trash2 className="mr-2 h-4 w-4" /> Hapus</Button></CardFooter></Card>))}
        </div>
      )}
      {isGoalFormOpen && (
        <DynamicDialog open={isGoalFormOpen} onOpenChange={(open) => { setIsGoalFormOpen(open); if (!open) resetGoalForm(); }}>
          <DynamicDialogContent className="sm:max-w-md" aria-labelledby={goalFormDialogTitleId}>
            ...
            <DynamicDialogFooter><DynamicDialogClose asChild><Button variant="outline">Batal</Button></DynamicDialogClose><Button onClick={handleSaveGoal} className="bg-primary hover:bg-primary/90 text-primary-foreground">Simpan</Button></DynamicDialogFooter>
      ...
      {isTransactionFormOpen && selectedGoalForTransaction && (
        <DynamicDialog open={isTransactionFormOpen} onOpenChange={(open) => { setIsTransactionFormOpen(open); if (!open) setSelectedGoalForTransaction(null); }}>
          <DynamicDialogContent className="sm:max-w-md" aria-labelledby={transactionFormDialogTitleId}>
            ...
            <DynamicDialogFooter><DynamicDialogClose asChild><Button variant="outline">Batal</Button></DynamicDialogClose><Button onClick={handleSaveTransaction} className="bg-primary hover:bg-primary/90 text-primary-foreground">Simpan</Button></DynamicDialogFooter>
          </DynamicDialogContent>
        </DynamicDialog>
      )}
