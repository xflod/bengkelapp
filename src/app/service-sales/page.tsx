...
export default function ServiceSalesPage() {
  ...
  React.useEffect(() => {
    const fetchProducts = async () => {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('is_active', true)
        .filter('selling_prices', 'cs', '[{"tierName":"default"}]'); // Ensure we fetch products with default prices for services
      
      if (error) {
        console.error("Error fetching products for service page (raw):", JSON.stringify(error, null, 2));
        ...
        if (error.message) {
          ...
          detailedMessage = "Query item servis berhasil namun tidak ada data yang dikembalikan, atau akses ditolak. Mohon periksa Row Level Security (RLS) di Supabase untuk tabel 'products' dan pastikan tabelnya ada serta item memiliki harga default.";
  ...
  const completeTransaction = async (paymentType: 'Tunai' | 'Transfer', details: string) => {
    ...
    const transactionForReportSupabase = {
      ...
      type: 'Service', created_at: transactionDate.toISOString(),
    };
