...
export default function PartnerSalesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-md"><CardHeader><CardTitle>Informasi Bengkel Mitra</CardTitle></CardHeader><CardContent><Label htmlFor="partnerName">Nama Bengkel Mitra</Label><Input id="partnerName" value={partnerName} onChange={(e) => setPartnerName(e.target.value)} placeholder="Masukkan nama bengkel mitra" className="mt-1"/></CardContent></Card>
      <div className="flex flex-col md:flex-row gap-6">
        <div className="md:w-2/3 space-y-4">
          <Card className="shadow-md"><CardHeader><CardTitle>Cari Produk (Harga Partner)</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex gap-2"><div className="relative flex-grow"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /><Input type="text" placeholder="Ketik kode, nama, atau scan barcode..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/></div><Button variant="outline" size="icon" onClick={() => setIsCameraOpen(!isCameraOpen)} title={isCameraOpen ? "Tutup Scanner" : "Buka Scanner"}>{isCameraOpen ? <XCircle className="h-5 w-5" /> : <Camera className="h-5 w-5" />}</Button></div>{isCameraOpen && (<div className="space-y-2"><video ref={videoRef} className="w-full aspect-video rounded-md bg-muted border" autoPlay muted playsInline />{hasCameraPermission === false && (<Alert variant="destructive"><AlertTitle>Kamera Dibutuhkan</AlertTitle><AlertDescription>Izinkan akses kamera. <i>(Simulasi: Ketik 'BARCODE123P')</i></AlertDescription></Alert>)}{hasCameraPermission === true && (<Alert><AlertTitle>Pemindai Aktif</AlertTitle><AlertDescription>Arahkan kamera ke barcode. <i>(Simulasi: Ketik 'BARCODE123P')</i></AlertDescription></Alert>)}</div>)}</CardContent></Card>
          <Card className="shadow-md"><CardHeader><CardTitle>Hasil Pencarian ({filteredProducts.length})</CardTitle></CardHeader><CardContent>{filteredProducts.length > 0 ? (<div className="max-h-[400px] overflow-y-auto"><Table><TableHeader><TableRow><TableHead>Nama Produk</TableHead><TableHead className="text-right">Harga Partner</TableHead><TableHead className="text-center">Stok</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{filteredProducts.map((product) => { const partnerPriceInfo = product.sellingPrices.find(p => p.tierName === 'partner'); return (<TableRow key={product.id}><TableCell className="font-medium break-words max-w-[150px] sm:max-w-xs">{product.name} <br/> <span className="text-xs text-muted-foreground">({product.sku} - {product.category})</span></TableCell><TableCell className="text-right whitespace-nowrap">Rp {partnerPriceInfo?.price.toLocaleString() || 'N/A'}</TableCell><TableCell className="text-center">{product.category === 'Jasa' ? '-' : product.stockQuantity}</TableCell><TableCell className="text-right"><Button size="icon" onClick={() => handleAddToCart(product)} disabled={(product.category !== 'Jasa' && product.stockQuantity <= 0) || !partnerPriceInfo} className="bg-primary hover:bg-primary/90 text-primary-foreground h-8 w-8" title="Tambah"><ShoppingCart className="h-4 w-4" /></Button></TableCell></TableRow>);})}</TableBody></Table></div>) : (<p className="text-muted-foreground text-center py-4">Tidak ada produk (partner) yang cocok.</p>)}</CardContent></Card>
        </div>
        <div className="md:w-1/3 space-y-4">
          <Card className="shadow-md md:sticky md:top-20"><CardHeader><CardTitle>Keranjang Belanja Partner</CardTitle><CardDescription>Total Item: {cart.reduce((sum, item) => sum + item.quantity, 0)}</CardDescription></CardHeader><CardContent className="space-y-4">{cart.length > 0 ? (<div className="max-h-[450px] overflow-y-auto"><Table><TableHeader><TableRow><TableHead>Produk</TableHead><TableHead className="text-center">Qty</TableHead><TableHead className="text-right">Total</TableHead><TableHead className="text-right">Aksi</TableHead></TableRow></TableHeader><TableBody>{cart.map(item => (<TableRow key={item.productId}><TableCell className="font-medium text-sm break-words max-w-[100px] sm:max-w-[80px] md:max-w-[100px] lg:max-w-xs">{item.productName}</TableCell><TableCell className="text-center"><div className="flex items-center justify-center gap-1"><Button variant="outline" size="icon" className="h-6 w-6" onClick={() => handleUpdateQuantity(item.productId, item.quantity - 1)}><MinusCircle className="h-3 w-3" /></Button><span className="w-6 text-center text-sm">{item.quantity}</span><Button variant="outline" size="icon" className="h-6 w-6" onClick={() => handleUpdateQuantity(item.productId, item.quantity + 1)}><PlusCircle className="h-3 w-3" /></Button></div></TableCell><TableCell className="text-right text-sm whitespace-nowrap">Rp {item.totalPrice.toLocaleString()}</TableCell><TableCell className="text-right"><Button variant="ghost" size="icon" onClick={() => handleRemoveFromCart(item.productId)} className="text-destructive hover:text-destructive/80 h-7 w-7"><Trash2 className="h-4 w-4" /></Button></TableCell></TableRow>))}</TableBody></Table></div>) : (<p className="text-muted-foreground text-center py-4">Keranjang partner kosong.</p>)}</CardContent>{cart.length > 0 && (<CardFooter className="flex flex-col space-y-2 pt-4 border-t"><div className="flex justify-between w-full text-md"><span>Subtotal (Partner):</span><span className="whitespace-nowrap">Rp {subtotalForCart.toLocaleString()}</span></div>{parsedDiscount > 0 && (<div className="flex justify-between w-full text-md text-green-600"><span>Diskon:</span><span className="whitespace-nowrap">- Rp {actualDiscountApplied.toLocaleString()}</span></div>)}<div className="flex justify-between w-full font-semibold text-lg"><span>Total Akhir (Partner):</span><span className="whitespace-nowrap">Rp {finalTotalForPayment.toLocaleString()}</span></div></CardFooter>)}</Card>
        </div>
      </div>
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Pembayaran Partner</DialogTitle><DialogDescription>Subtotal: <span className="font-semibold">Rp {subtotalForCart.toLocaleString()}</span>{parsedDiscount > 0 && (<><br/>Diskon: <span className="font-semibold text-green-600">- Rp {actualDiscountApplied.toLocaleString()}</span></>)}<br/>Total: <span className="font-bold text-lg">Rp {finalTotalForPayment.toLocaleString()}</span></DialogDescription></DialogHeader><div className="space-y-2 pt-2"><Label htmlFor="discountAmountPartner">Diskon (Rp)</Label><div className="relative"><Percent className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input id="discountAmountPartner" type="number" value={discountAmount} onChange={(e) => { const v = e.target.value; const nV = parseFloat(v); if (v === "" || (nV >= 0 && nV <= subtotalForCart) ) { setDiscountAmount(v); } else if (nV > subtotalForCart) { setDiscountAmount(subtotalForCart.toString()); toast({title: "Info", description: "Diskon maks. subtotal."}); } }} placeholder="e.g. 5000" className="pl-10 text-base w-full"/></div></div><Tabs value={paymentMethodTab} onValueChange={(value) => setPaymentMethodTab(value as 'cash' | 'transfer')} className="w-full pt-2"><TabsList className="grid w-full grid-cols-2"><TabsTrigger value="cash"><HandCoins className="mr-2 h-4 w-4" />Tunai</TabsTrigger><TabsTrigger value="transfer"><Landmark className="mr-2 h-4 w-4" />Transfer</TabsTrigger></TabsList><TabsContent value="cash" className="space-y-4 pt-4"><div className="space-y-2"><Label htmlFor="cashReceivedPartner">Uang Diterima (Rp)</Label><Input id="cashReceivedPartner" type="number" value={cashReceived} onChange={(e) => setCashReceived(e.target.value)} placeholder="e.g. 150000" className="text-base w-full"/></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => handlePresetCash(finalTotalForPayment)} className="flex-1 text-xs min-w-[80px]">Uang Pas</Button><Button variant="outline" onClick={() => handlePresetCash(50000)} className="flex-1 text-xs min-w-[80px]">Rp 50rb</Button><Button variant="outline" onClick={() => handlePresetCash(100000)} className="flex-1 text-xs min-w-[80px]">Rp 100rb</Button></div><div className="space-y-1 text-right"><p className="text-sm">Total: Rp {finalTotalForPayment.toLocaleString()}</p><p className="text-sm">Diterima: Rp {(parseFloat(cashReceived) || 0).toLocaleString()}</p><p className="text-lg font-semibold">Kembali: Rp {changeCalculated >= 0 ? changeCalculated.toLocaleString() : '0'}</p>{(parseFloat(cashReceived) || 0) < finalTotalForPayment && cashReceived !== "" && (<p className="text-sm text-destructive">Uang kurang.</p>)}</div><Button onClick={confirmCashPayment} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground" disabled={(parseFloat(cashReceived) || 0) < finalTotalForPayment}>Konfirmasi Tunai</Button></TabsContent><TabsContent value="transfer" className="space-y-4 pt-4"><p className="text-sm text-muted-foreground">Transfer <span className="font-semibold text-foreground">Rp {finalTotalForPayment.toLocaleString()}</span> ke:</p><div className="p-3 bg-muted rounded-md text-sm"><p className="font-medium">Bank XYZ</p><p>No. Rek: <span className="font-semibold">123-456-7890</span> A/N: <span className="font-semibold">BengkelKu Jaya (Partner)</span></p></div><Button onClick={confirmTransferPayment} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">Sudah Transfer</Button></TabsContent></Tabs><DialogFooter className="sm:justify-start mt-4"><DialogClose asChild><Button type="button" variant="outline" className="w-full sm:w-auto">Batal</Button></DialogClose></DialogFooter></DialogContent></Dialog>
      <Dialog open={isReceiptModalOpen} onOpenChange={setIsReceiptModalOpen}><DialogContent className="sm:max-w-sm w-[90vw]"><DialogHeader><DialogTitle>{receiptDetails?.receiptTitle || "Nota Transaksi"}</DialogTitle><DialogDescription>ID: {receiptDetails?.transactionId}</DialogDescription></DialogHeader><div ref={receiptRef} id="receipt-content" className="p-4 border rounded-md bg-white text-black text-xs"><div className="text-center mb-2"><h3 className="font-bold text-sm">BengkelKu App (Partner)</h3><p>Jl. Otomotif No. 1, Kota Maju</p><p>Telp: 0812-3456-7890</p></div><hr className="my-1 border-dashed border-gray-400"/><p>No: {receiptDetails?.transactionId}</p><p>Tgl: {receiptDetails?.date}</p>{receiptDetails?.customerName && <p>Partner: {receiptDetails.customerName}</p>}<hr className="my-1 border-dashed border-gray-400"/><table className="w-full my-1 table-fixed"><thead><tr><th className="text-left font-normal w-[40%]">Item</th><th className="text-center font-normal w-[15%]">Qty</th><th className="text-right font-normal w-[22.5%]">Harga</th><th className="text-right font-normal w-[22.5%]">Total</th></tr></thead><tbody>{receiptDetails?.items.map(item => (<tr key={item.productId}><td className="py-0.5 break-words">{item.productName}</td><td className="text-center py-0.5">{item.quantity}</td><td className="text-right py-0.5 break-words">{item.unitPrice.toLocaleString()}</td><td className="text-right py-0.5 break-words">{item.totalPrice.toLocaleString()}</td></tr>))}</tbody></table><hr className="my-1 border-dashed border-gray-400"/><div className="text-right"><p>Subtotal: <span className="font-semibold">Rp {receiptDetails?.subtotal.toLocaleString()}</span></p>{receiptDetails && receiptDetails.discount > 0 && (<p>Diskon: <span className="font-semibold">Rp {receiptDetails.discount.toLocaleString()}</span></p>)}<p className="font-bold">Total Bayar: <span className="font-bold">Rp {receiptDetails?.finalTotal.toLocaleString()}</span></p>{receiptDetails?.paymentMethod === 'Tunai' && (<><p>Tunai: Rp {receiptDetails?.cashReceived?.toLocaleString()}</p><p>Kembali: Rp {receiptDetails?.changeCalculated?.toLocaleString()}</p></>)}<p>Metode: {receiptDetails?.paymentMethod}</p></div><hr className="my-1 border-dashed border-gray-400"/><p className="text-center mt-2">Terima Kasih atas Kerjasamanya!</p><p className="text-center text-[0.6rem]">Layanan & Suku Cadang Motor (Mitra)</p></div><DialogFooter className="flex flex-col sm:flex-row sm:justify-end gap-2 mt-4"><Button onClick={handleDownloadReceipt} variant="outline" className="w-full sm:w-auto"><Download className="mr-2 h-4 w-4" /> Unduh (PNG)</Button><Button onClick={handleShareToWhatsApp} variant="outline" className="w-full sm:w-auto"><Share2 className="mr-2 h-4 w-4" /> WA</Button><DialogClose asChild><Button type="button" className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground">Tutup</Button></DialogClose></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
