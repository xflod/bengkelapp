...
export default function EmployeesPage() {
  ...
  const [loanDateState, setLoanDateState] = useState<Date | undefined>(new Date()); // Renamed to avoid conflict
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data: empData, error: empError } = await supabase.from('employees').select('*').order('createdAt', { ascending: false });
      if (empError) throw empError;
      setEmployees(empData as Employee[]);

      const { data: loanData, error: loanError } = await supabase.from('employeeLoans').select('*').order('createdAt', { ascending: false });
      if (loanError) throw loanError;
      setLoans(loanData as Loan[]);

      const { data: instData, error: instError } = await supabase.from('loanInstallments').select('*').order('createdAt', { ascending: false });
      if (instError) throw instError;
      setInstallments(instData as LoanInstallment[]);
  ...
  const resetEmployeeFormFields = useCallback(() => { /* Unchanged */ setEditingEmployee(null); setEmployeeName(''); setPosition(''); setJoinDate(new Date()); setPhone(''); setAddress(''); setEmployeeStatus('Aktif'); setPayrollFrequency('Bulanan'); setBaseSalary(''); setLoanNotes(''); setPerformanceNotes(''); }, []);
  const handleOpenEmployeeFormDialog = useCallback((employee?: Employee) => { /* Unchanged */ if (employee) { setEditingEmployee(employee); setEmployeeName(employee.name); setPosition(employee.position); setJoinDate(employee.joinDate && isValid(parseISO(employee.joinDate)) ? parseISO(employee.joinDate) : new Date()); setPhone(employee.phone || ''); setAddress(employee.address || ''); setEmployeeStatus(employee.status); setPayrollFrequency(employee.payrollFrequency); setBaseSalary(employee.baseSalary); setLoanNotes(employee.loanNotes || ''); setPerformanceNotes(employee.performanceNotes || ''); } else { resetEmployeeFormFields(); } setIsFormDialogOpen(true); }, [resetEmployeeFormFields]);
  ...
  const handleSaveEmployee = async () => {
    ...
    const employeeDataToSave = {
      name: employeeName.trim(), position: position.trim(), joinDate: format(joinDate, 'yyyy-MM-dd'),
      phone: phone.trim() || undefined, address: address.trim() || undefined, status: employeeStatus,
      payrollFrequency: payrollFrequency, baseSalary: parsedBaseSalary, loanNotes: loanNotes.trim() || undefined,
      performanceNotes: performanceNotes.trim() || undefined, updatedAt: now,
      // Supabase client-side generated id for employee table might not be needed if using auto-increment.
      // Let's assume 'id' is auto-generated by Supabase or we use a UUID if that's the Supabase setup.
      // For now, if editingProduct, we use its id. For new, Supabase handles it.
    };
    ...
    if (editingEmployee && editingEmployee.id) {
      ...
      const { error } = await supabase.from('employees').insert([{ ...employeeDataToSave, createdAt: now }]);
  ...
  const handleDeleteEmployee = useCallback(async (employeeId: number, name: string) => {
    if (window.confirm(`Yakin ingin menghapus karyawan "${name}"? Ini juga akan menghapus data pinjaman terkait.`)) {
      // Consider Supabase cascade delete or manual deletion of related loans/installments in a transaction (RPC)
      const { error: instError } = await supabase.from('loanInstallments').delete().in('loanId', loans.filter(l => l.employeeId === employeeId).map(l => l.id));
      const { error: loanError } = await supabase.from('employeeLoans').delete().match({ employeeId: employeeId });
      const { error: empError } = await supabase.from('employees').delete().match({ id: employeeId });

      if (empError || loanError || instError) toast({ variant: "destructive", title: "Gagal Hapus Karyawan/Data Terkait", description: empError?.message || loanError?.message || instError?.message });
      else { toast({ title: "Karyawan Dihapus" }); fetchData(); }
    }
  }, [toast, fetchData, loans]);
  
  const handleToggleEmployeeStatus = useCallback(async (employeeId: number, currentStatus: EmployeeStatus) => {
    const newStatus: EmployeeStatus = currentStatus === 'Aktif' ? 'Tidak Aktif' : 'Aktif';
    const { error } = await supabase.from('employees').update({ status: newStatus, updatedAt: new Date().toISOString() }).match({ id: employeeId });
  ...
  const openLoanFormDialog = (loan?: Loan) => { if (loan) { setEditingLoan(loan); setLoanDateState(loan.loanDate && isValid(parseISO(loan.loanDate)) ? parseISO(loan.loanDate) : new Date()); setLoanAmount(loan.loanAmount); setLoanReason(loan.reason || ''); setLoanRepaymentNotes(loan.repaymentNotes || ''); } else { resetLoanFormFields(); } setIsLoanFormDialogOpen(true); };
  ...
  const handleSaveLoan = async () => {
    ...
    if (editingLoan && editingLoan.id) {
        const updatedLoanData = {
            loanDate: format(loanDateState, 'yyyy-MM-dd'), loanAmount: parsedLoanAmount,
            reason: loanReason.trim() || undefined, repaymentNotes: loanRepaymentNotes.trim() || undefined,
            remainingBalance: parsedLoanAmount - (editingLoan.loanAmount - editingLoan.remainingBalance), 
            updatedAt: now,
            status: 'Aktif' as LoanStatus, // Default, will be updated based on remainingBalance
        };
        if (updatedLoanData.remainingBalance < 0) updatedLoanData.remainingBalance = 0;
        updatedLoanData.status = updatedLoanData.remainingBalance === 0 ? 'Lunas' : 'Aktif';
        
        const { error } = await supabase.from('employeeLoans').update(updatedLoanData).match({ id: editingLoan.id });
        ...
        const newLoanData = {
            employeeId: selectedEmployeeForLoan.id, loanDate: format(loanDateState, 'yyyy-MM-dd'),
            loanAmount: parsedLoanAmount, reason: loanReason.trim() || undefined,
            repaymentNotes: loanRepaymentNotes.trim() || undefined, status: 'Aktif' as LoanStatus,
            remainingBalance: parsedLoanAmount, createdAt: now, updatedAt: now,
        };
        const { error } = await supabase.from('employeeLoans').insert([newLoanData]);
  ...
  const handleDeleteLoan = async (loanId: number) => {
    if (window.confirm("Yakin hapus pinjaman ini? Semua cicilan terkait juga akan dihapus.")) {
        ...
        await supabase.from('loanInstallments').delete().match({ loanId: loanId });
        const { error } = await supabase.from('employeeLoans').delete().match({ id: loanId });
  ...
  const handleSaveInstallment = async () => {
    ...
    if (parsedInstallmentAmount > selectedLoanForInstallment.remainingBalance) { toast({ variant: "destructive", title: "Cicilan Melebihi Sisa"}); return; }

    const now = new Date().toISOString();
    const newInstallmentData = {
        loanId: selectedLoanForInstallment.id, paymentDate: format(installmentDate, 'yyyy-MM-dd'),
        amountPaid: parsedInstallmentAmount, notes: installmentNotes.trim() || undefined, createdAt: now,
    };
    const { error: instError } = await supabase.from('loanInstallments').insert([newInstallmentData]);
    if (instError) { toast({ variant: "destructive", title: "Gagal Simpan Cicilan", description: instError.message }); return; }

    const newRemaining = selectedLoanForInstallment.remainingBalance - parsedInstallmentAmount;
    let newStatus: LoanStatus = 'Aktif';
    if (newRemaining <= 0) newStatus = 'Lunas';
    else if (newRemaining < selectedLoanForInstallment.loanAmount) newStatus = 'Sebagian Lunas';
    
    const { error: loanUpdateError } = await supabase.from('employeeLoans')
      .update({ remainingBalance: Math.max(0, newRemaining), status: newStatus, updatedAt: now })
  ...
  // UI helper functions (getLoanStatusBadgeColor, filteredEmployees, getStatusBadgeColor) remain the same
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Employee Dialog */}
      <Dialog open={isFormDialogOpen} onOpenChange={(open) => { setIsFormDialogOpen(open); if (!open) resetEmployeeFormFields(); }}><DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col"><DialogHeader className="flex-shrink-0"><DialogTitle>{editingEmployee ? 'Edit Karyawan' : 'Tambah Karyawan'}</DialogTitle><DialogDescription>{editingEmployee ? `Mengedit ${editingEmployee.name}.` : 'Detail karyawan baru.'}</DialogDescription></DialogHeader><div className="grid gap-y-3 gap-x-4 py-2 flex-grow overflow-y-auto pr-3 text-sm">
      ...
      </div><DialogFooter className="flex-shrink-0 pt-4 border-t"><DialogClose asChild><Button type="button" variant="outline">Batal</Button></DialogClose><Button type="button" onClick={handleSaveEmployee} className="bg-primary hover:bg-primary/90 text-primary-foreground">Simpan</Button></DialogFooter></DialogContent></Dialog>
      {/* Loan Management Dialog */}
      <Dialog open={isLoanMgmtDialogOpen} onOpenChange={setIsLoanMgmtDialogOpen}><DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col"><DialogHeader><DialogTitle>Man. Pinjaman: {selectedEmployeeForLoan?.name}</DialogTitle><DialogDescription>Kelola pinjaman & cicilan.</DialogDescription></DialogHeader><div className="flex-grow overflow-y-auto p-1"><Button onClick={() => openLoanFormDialog()} className="mb-4 bg-accent hover:bg-accent/90 text-accent-foreground"><PlusCircle className="mr-2 h-4 w-4" /> Tambah Pinjaman</Button>
        {loans.filter(l => l.employeeId === selectedEmployeeForLoan?.id).length > 0 ? (<Table><TableHeader><TableRow><TableHead>Tgl Pinjam</TableHead><TableHead className="text-right">Jumlah</TableHead><TableHead className="text-right">Sisa</TableHead><TableHead className="text-center">Status</TableHead><TableHead className="text-center">Aksi</TableHead></TableRow></TableHeader><TableBody>
          {loans.filter(l => l.employeeId === selectedEmployeeForLoan?.id).sort((a,b) => new Date(b.loanDate).getTime() - new Date(a.loanDate).getTime()).map(loan => (<TableRow key={loan.id}><TableCell>{format(parseISO(loan.loanDate), "dd MMM yyyy", { locale: localeID })}</TableCell><TableCell className="text-right">Rp {loan.loanAmount.toLocaleString()}</TableCell><TableCell className="text-right">Rp {loan.remainingBalance.toLocaleString()}</TableCell><TableCell className="text-center"><Badge className={`${getLoanStatusBadgeColor(loan.status)} text-white`}>{loan.status}</Badge></TableCell><TableCell className="text-center space-x-1"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => openLoanFormDialog(loan)} title="Edit"><Edit className="h-4 w-4"/></Button><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => openInstallmentDialog(loan)} title="Cicilan"><CircleDollarSign className="h-4 w-4"/></Button><Button variant="destructive" size="icon" className="h-7 w-7" onClick={() => handleDeleteLoan(loan.id)} title="Hapus"><Trash2 className="h-4 w-4"/></Button></TableCell></TableRow>))}
        </TableBody></Table>) : (<p className="text-muted-foreground text-center py-4">Belum ada data pinjaman.</p>)}
      </div><DialogFooter><DialogClose asChild><Button variant="outline">Tutup</Button></DialogClose></DialogFooter></DialogContent></Dialog>
      {/* Add/Edit Loan Dialog */}
      <Dialog open={isLoanFormDialogOpen} onOpenChange={setIsLoanFormDialogOpen}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{editingLoan ? 'Edit' : 'Tambah'} Pinjaman u/ {selectedEmployeeForLoan?.name}</DialogTitle></DialogHeader><div className="grid gap-4 py-4">
      ...
      </div><DialogFooter><DialogClose asChild><Button variant="outline">Batal</Button></DialogClose><Button onClick={handleSaveLoan} className="bg-primary hover:bg-primary/90 text-primary-foreground">Simpan</Button></DialogFooter></DialogContent></Dialog>
      {/* Installment Dialog */}
      {selectedLoanForInstallment && (<Dialog open={isInstallmentDialogOpen} onOpenChange={setIsInstallmentDialogOpen}><DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col"><DialogHeader><DialogTitle>Cicilan Pinjaman - {selectedEmployeeForLoan?.name}</DialogTitle><DialogDescription>Pinjaman: Rp {selectedLoanForInstallment.loanAmount.toLocaleString()} | Sisa: Rp {selectedLoanForInstallment.remainingBalance.toLocaleString()}<br/>Tgl Pinjam: {format(parseISO(selectedLoanForInstallment.loanDate), "dd MMM yyyy", { locale: localeID })}</DialogDescription></DialogHeader><div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto p-1">
        <div className="space-y-4 border-r-0 md:border-r md:pr-6"><h3 className="text-md font-semibold">Tambah Cicilan</h3>
        ...
        <div className="space-y-2"><h3 className="text-md font-semibold">Riwayat Cicilan</h3>
          {installments.filter(inst => inst.loanId === selectedLoanForInstallment.id).length > 0 ? (<div className="max-h-60 overflow-y-auto"><Table><TableHeader><TableRow><TableHead>Tgl Bayar</TableHead><TableHead className="text-right">Jumlah</TableHead><TableHead>Catatan</TableHead></TableRow></TableHeader><TableBody>
            {installments.filter(inst => inst.loanId === selectedLoanForInstallment.id).sort((a,b) => new Date(b.paymentDate).getTime() - new Date(a.paymentDate).getTime()).map(inst => (<TableRow key={inst.id}><TableCell>{format(parseISO(inst.paymentDate), "dd MMM yy", { locale: localeID })}</TableCell><TableCell className="text-right">Rp {inst.amountPaid.toLocaleString()}</TableCell><TableCell className="text-xs">{inst.notes || '-'}</TableCell></TableRow>))}
          </TableBody></Table></div>) : (<p className="text-muted-foreground text-center text-sm py-4">Belum ada cicilan.</p>)}
        </div>
      </div><DialogFooter className="mt-4"><DialogClose asChild><Button variant="outline">Tutup</Button></DialogClose></DialogFooter></DialogContent></Dialog>)}
    </div>
  );
}
