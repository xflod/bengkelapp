...
export default function EmployeesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setLoans(loanData.map(l => ({...l, id: String(l.id), employeeId: String(l.employee_id)})) as Loan[]);

      const { data: instData, error: instError } = await supabase.from('loan_installments').select('*').order('created_at', { ascending: false });
      if (instError) throw instError;
      setInstallments(instData.map(i => ({...i, id: String(i.id), loanId: String(i.loan_id)})) as LoanInstallment[]);
  ...
  const handleDeleteEmployee = useCallback(async (employeeId: string, name: string) => {
    if (window.confirm(`Yakin ingin menghapus karyawan "${name}"? Ini juga akan menghapus data pinjaman terkait.`)) {
      // Supabase cascade delete should handle this if set up. Otherwise, manual:
      const employeeLoansToDelete = loans.filter(l => l.employeeId === employeeId).map(l => l.id);
  ...
  const handleSaveLoan = async () => {
    ...
    if (editingLoan && editingLoan.id) {
        const updatedLoanData = {
            ...
            remaining_balance: parsedLoanAmount - (editingLoan.loanAmount - editingLoan.remainingBalance), 
            updated_at: now,
